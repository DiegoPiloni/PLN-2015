...
from parsing.features import (History, distance, top_of_stack_word, top_of_stack_pos, 
...
class TBDependencyParser():
    ...
    def __init__(self, parsed_sents, classifier='svm'):
        """
        parsed_sents -- list of sentences (each one a list of lists [[Word, Root, Pos, Head]].
    ...
    def sent_histories_actions(self, parsed_sent):
        ...
        parsed_sent -- the parsed sentence (a list of lists [word, root, postag, head]).
        """
        ...
        while hist_buf:
            if len(hist_stack) > 1:
                if complete_buf[0][4] == complete_stack[-1][0]: # RARC
                ...
                elif complete_stack[-1][4] == complete_buf[0][0]: # LARC
                    hs.append(History(hist_stack, hist_buf))
                    actions.append("LEFT ARC")
                    hist_stack = hist_stack[:-1]
                    complete_stack = complete_stack[:-1]
                else: # Not related
                    # check if any of the stack elems is related with top of buf
